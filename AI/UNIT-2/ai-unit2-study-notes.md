...
*   **Complexities:**
    *   **Time & Space Complexity:** $O(b^{1 + \lfloor C^* / \epsilon floor})$, where $C^*$ is the cost of the optimal solution and $\epsilon$ is the minimum action cost [22, 59, 122, 143, 162, 183, 201]. This can be much larger than $b^d$ if step costs are highly non-uniform [22, 59, 122, 143, 162, 183, 201].
...
| **Time Complexity** | $O(b^d)$ [40, 184, 203] | $O(b^m)$ [184, 203] | $O(b^{1 + \lfloor C^* / \epsilon floor})$ [22, 59, 122, 143, 162, 183, 201] |
| **Space Complexity** | $O(b^d)$ [40, 184, 203] | $O(b \cdot m)$ [30, 66, 130, 151, 170, 184, 203] | $O(b^{1 + \lfloor C^* / \epsilon floor})$ [22, 59, 122, 143, 162, 183, 201] |
...
7.  **BFS Path Returned:** S $ightarrow$ B $ightarrow$ F $ightarrow$ G (Step Count = 3).
...
**Progress:** Arad ($366$) $ightarrow$ Sibiu ($253$) $ightarrow$ Fagaras ($176$) $ightarrow$ Bucharest ($0$) [2, 3, 60]. Finds path without expanding any other branch, but the path is not optimal if the actual roads are circuitous.
...
### 3.4 Worked Example: Informed Search Trace (S $ightarrow$ G)
...
*   **UCS/BFS Time/Space Complexities:** BFS $= O(b^d)$ [40, 184, 203], UCS $= O(b^{1 + \lfloor C^* / \epsilon floor})$ [22, 59, 122, 143, 162, 183, 201].
...
*   *Answer:* **No, it is mathematically impossible.** Every consistent heuristic is guaranteed to be admissible. We prove this by induction on the path length to the goal.
    *   Let $n_k ightarrow n_{k-1} ightarrow \dots ightarrow n_0$ be the optimal path from node $n_k$ to the goal $n_0$ (where $h(n_0)=0$).
