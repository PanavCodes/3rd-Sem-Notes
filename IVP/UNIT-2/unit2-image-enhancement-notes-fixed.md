<!-- Cleaned version: Markdown/math delimiters and obvious formatting artifacts repaired. -->
...
* **Point Processing (Pixel-by-Pixel):** The neighborhood around pixel $(x, y)$has a size of$1 \times 1$. The output intensity $s$ at any coordinate depends *only* on the input intensity$r$ at that exact coordinate.
    * *Equation:* $s = T(r)$
* **Neighborhood Processing (Filtering):** The neighborhood is larger (e.g., $3 \times 3$or$5 \times 5$). The output intensity $g(x, y)$ is computed based on both the center pixel and its surrounding neighbors.
...
* If **Slope $> 1$** (typically region $\beta$): The intensity range is **stretched** (contrast is increased).
* If **Slope $< 1$** (regions $\alpha$and$\gamma$): The intensity range is **compressed**.
...
s = \begin{cases} L-1 & \text{if } r > T \\ 0 & \text{if } r \le T \end{cases}
...
* **Iterative Auto-Threshold Estimation Algorithm:**
    ...
    2. Segment the image using $T$ to produce two classes of pixels:$G_1$ (pixels$> T$) and $G_2$ (pixels$\le T$).
    3. Compute mean intensities $\mu_1$and$\mu_2$for$ G_1$and$ G_2$ respectively.
    4. Compute a new threshold $T_{\text{new}} = \frac{\mu_1 + \mu_2}{2}$.
...
* **Step 1: Compute Highlight Range:**
    ...
    * Target band of interest: $r \in \{3, 4\}$.
...
* **Mathematical Formula:**
    $s = c \cdot r^{\gamma}$
* **Fractional Gamma ($\gamma < 1$):** Behaves like a log transform—brightens the dark shadow regions and expands dynamic contrast.
* **Integer Gamma ($\gamma > 1$):** Maps bright highlight levels into a wider dynamic range, darkening the image.
...
* **Transformation Function:**
  $$s_k = T(r_k) = (L - 1) \cdot \sum_{j=0}^{k} p_r(r_j) = (L - 1) \cdot \text{CDF}(r_k)$$
    where:
    * $p_r(r_j) = \frac{n_j}{N}$ is the Probability Density Function (PDF).
    * $N$ is the total number of pixels in the image.
    * $\text{CDF}(r_k)$ is the Cumulative Distribution Function.
...
Neighborhood operations (filtering) modify the intensity of a pixel based on the intensities of surrounding pixels inside a small local sub-grid (often $3 \times 3$or$5 \times 5$).
...
> *Question:* Given the grayscale image segment below, apply a $3 \times 3$ mask for the following operations on the inner pixels. Ignore rows and columns at the extreme edges.
> 
> $$
...
* **Piecewise Linear Slopes:**
    $\alpha = \frac{s_1}{r_1}, \quad \beta = \frac{s_2 - s_1}{r_2 - r_1}, \quad \gamma = \frac{(L-1) - s_2}{(L-1) - r_2}$
* **Log Transform:** $s = c \log_{10}(1 + r)$
* **Power-Law Transform:** $s = c \cdot r^{\gamma}$
